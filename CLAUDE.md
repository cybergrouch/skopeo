...
**Web UI** (`web/`, React + Vite, generated API client under `web/src/api/generated/`): Firebase-auth sign-up/login, then a capability-gated dashboard (`routes/DashboardPage.tsx`) with Profile / Research / Standings / Event Organizer / Seeding / Ratings / Invites / Activity Log / Admin tabs. Gating (`web/src/auth/capabilities.ts`): Event Organizer + Seeding require match-management (HOST/CLUB_OWNER/ADMIN); Research requires RESEARCHER/ADMIN; Ratings require RATER/ADMIN; Invites/Activity Log/Admin require ADMINISTRATOR. Public pages (`/players/:code`, `/matches/:code`, `/events/:code`) with QR share sit outside the dashboard. See `docs/engineering/architecture/WEB_UI_ARCHITECTURE.md`.
