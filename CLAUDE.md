...
**Web UI** (`web/`, React + Vite, generated API client under `web/src/api/generated/`): Firebase-auth sign-up/login, then a capability-gated dashboard (`routes/DashboardPage.tsx`) with Profile / Settings / Research / Standings / Claim / Event Organizer / Seeding / Placeholder Players / Ratings / Invites / Activity Log / Reports / Points Management / Account Management / Club Management / Admin / About tabs. Gating (`web/src/auth/capabilities.ts`): Settings is PLAYER (every user); Event Organizer + Seeding + Placeholder Players require match-management (HOST/CLUB_OWNER/ADMIN); Research requires RESEARCHER/ADMIN; Ratings require RATER/ADMIN; Points Management requires POINTS_MANAGER/ADMIN; Invites/Activity Log/Reports/Account Management/Club Management/Admin require ADMINISTRATOR; Profile/Standings/Claim/About are always shown. Public pages (`/players/:code`, `/matches/:code`, `/events/:code`) with QR share sit outside the dashboard. See `docs/engineering/architecture/WEB_UI_ARCHITECTURE.md`.
