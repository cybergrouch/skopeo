...
- **Users**: `routes/UserRoutes.kt` → `service/user/UserService.kt` → `repository/UserRepository.kt` (`UserTables.kt`). Sign-up (`POST /api/v1/users`) provisions a profile from the verified Firebase token; `sex` (Male/Female) and `dateOfBirth` are required. Names and contacts are append-only sub-resources (`NameRoutes`/`ContactRoutes`). Authorization is capability-based (`Capability`: PLAYER, HOST, CLUB_OWNER, ADMINISTRATOR, RATER, RESEARCHER, POINTS_MANAGER, SCORER, ACCOUNT_MANAGER) via `CapabilityRoutes`/`CapabilityService`. Role *sets* live in one place, `common/security/CapabilityRoles.kt` (#866) — note the **view/action split** (#1002): ACCOUNT_MANAGER joins every view set (`EMAIL_VIEW_ROLES`, `PLAYER_POINTS_VIEW_ROLES`, `PLAYER_SEARCH_ROLES`) and no action set, and `PLAYER_SEARCH_ROLES` is now `STAFF_ROLES` = `Capability.entries - PLAYER`, where dropping that subtraction would grant player search to every signed-in user.
...
- **Matches**: `routes/MatchRoutes.kt` → `service/match/MatchService.kt` → `repository/MatchRepository.kt` (`MatchTables.kt`). Staff create fixtures and upload results (scoped by the club rule below); recording a result does NOT compute ratings.
- **Club-scoped event/match authorization (#789)**: `service/club/ClubAccess.kt` is the ONE place `club_owners` is consulted for authorization. `mayOrganize(caller, event)` = ADMINISTRATOR **or** a named owner of `event.clubId` **or** `event.createdBy == caller.id` (grandfathered permanently — no migration, and it is also the whole rule for a clubless "Open" event). Every per-event gate in `EventService`/`EventTeamService`, and the event-scoped operations in `MatchService` (fixture create, uploadResult, setHandicaps, setActive, reorder), go through it. `mayFileUnder` is the one *tightening*: creating or re-filing an event under a club requires owning that club. `GET /events` is scoped to owned-club events **plus** own-created ones (`EventRepository.listForOrganizer`). The event-expiry gate (#310, `ensureHostMayEnter`) is a separate axis and also applies.
...
**Web UI** (`web/`, React + Vite, generated API client under `web/src/api/generated/`): Firebase-auth sign-up/login, then a capability-gated dashboard (`routes/DashboardPage.tsx`) with Profile / Settings / Research / Standings / Seeding / Placeholder Players / Ratings / Activity Log / Reports / Points Management / Account Management / Club Management / Admin / About tabs. Gating (`web/src/auth/capabilities.ts`, plus `web/src/auth/clubAccess.ts` for per-club ownership, #789): Settings is PLAYER (every user); Seeding + Placeholder Players require match-management (HOST/CLUB_OWNER/ADMIN); the Event Organizer tab was removed in #794 — every club organizes from its own public page; Research requires RESEARCHER/ADMIN; Ratings require RATER/ADMIN; Points Management requires POINTS_MANAGER/ADMIN; Club Management requires match-management (HOST/CLUB_OWNER/ADMIN, #786) for *visibility*, with each operation inside keeping its own server rule (create/rename/delete/owners ADMINISTRATOR, sanctioning CLUB_OWNER/ADMIN); Activity Log/Reports/Admin require ADMINISTRATOR; Account Management requires ACCOUNT_MANAGER/ADMIN (#1002), except its `ManagePlayerSection` card which stays ADMINISTRATOR-only — its controls answer to other rules (ratings, points, capability grants, account delete), so showing it would offer an account manager controls that 403 and would put a route to ADMINISTRATOR on the page; Profile/Standings/About are always shown. Onboarding invites live inside the Account Management tab (#725, folded back in from a standalone tab). Claiming a placeholder account (#496) has no standalone tab — it appears conditionally on the Profile tab, shown only while the owner's account is claim-eligible (no rating and no match history) and hidden once claimed (#727). Public pages (`/players/:code`, `/matches/:code`, `/events/:code`, `/clubs/:code`) with QR share sit outside the dashboard; the club page's New Event form renders only for a named owner of THAT club plus administrators (#789). See `docs/engineering/architecture/WEB_UI_ARCHITECTURE.md`.
...
**"NTRP" is our own defined term and is never spelled out** (#879): the expanded phrase is a USTA trademark. It means a 1.0–7.0 scale in 0.5-point bands — a reference framework loosely based on USTA's rating system, not an implementation of it. Credit prior art freely ("the shape we borrowed from USTA's rating system"), but never write anything implying we implement their programme or reproduce their numbers ("USTA standard", "USTA-style", "implements USTA…"). The canonical definition is the NTRP entry in `docs/product/RATING_CALCULATION_ALGORITHM.md`'s glossary; defer to it rather than paraphrasing. The web UI's non-affiliation disclaimer (`web/src/components/NtrpLabel.tsx`, #842) deliberately *does* name the organization — that is the point of a disclaimer — and must not be reworded.

**Algorithm behavior** (details in `docs/product/RATING_CALCULATION_ALGORITHM.md`): single NTRP K-factor of 0.16 over the 1.0–7.0 range. Rating changes depend on dominance (game margin), the rating gap vs a competitive threshold (8.3% of the NTRP range = 0.5 points; expected wins beyond it yield zero change), a 2× upset multiplier, and optional smoothing — averaging the new rating against the previous one — via `options.smoothingFactor`. **Calibration (#881):** for the first N rated matches after a rating is assigned by hand (N is a global admin setting, default 10), the player's own rating moves but their opponents'/partners' do not — unless those players are calibrating too. Suppression lives in `RatingCalculationService`, never in the calculator, which stays pure and zero-sum; the *pool* is therefore not conserved while anyone is calibrating. A suppressed player gets no rating row and no history row, which is what lets a score correction reverse only what was applied. `CalibrationService` is the one place the rule is evaluated, and it is derived (designation + rated-match count + current N), never stored.
...
- Layered package dependencies are enforced by `LayeredArchitectureTest` (ArchUnit): `repository` is pure data-access returning raw `persistence` entities, `persistence`/`model`/`common` are leaves, the two mapper packages `mapper.dto` (dto↔model) and `mapper.entity` (entity↔model) are consumed only by `service`, `dto` never touches `routes`/`repository`, `service` never touches `routes`. See `docs/engineering/architecture/LAYERED_ARCHITECTURE.md`. (#633 full-separation flip rolls out aggregate-by-aggregate.)
