...
| **MatchService** | ~~`matchType`/`matchFormat` enums, `matchDate` date, team-id UUIDs, composition (players/side); score shape (non-negative games, ≥1 set)~~ ✅ migrated | staff-only; participants/staff read; not-disabled; not-already-completed; rated-lock; participant existence/active/rated. Outcome derivation stays (single-set-vs-stored-format, decisive set, sets-not-tied → winner) — it validates the score *against the persisted match*, not pure request shape. **#108 will rework this surface** (removes `matchFormat`). |
