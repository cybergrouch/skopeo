...
| `service/standings/StandingsService.kt` (`StandingsView`, line 54 — the `revealRates` field) | Data-class `copy`-default-argument synthetic | Compiler-generated `copy$default` bytecode for the internal view data class (`copy` is called with a subset of fields at the empty-view site, so the defaulted-field bridge is only partially exercised). JaCoCo attributes ~3 synthetic instructions to the last field's line. Would be excluded if it lived under `model/`; it earns its place beside the service, so the synthetic is accepted — do not add a contrived `copy()`/destructuring test for it. |
