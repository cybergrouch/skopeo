...
| `service/standings/StandingsService.kt` (`StandingsView`, ~line 53) | Data-class synthetics (`revealRates`/`copy` component + default-arg path) | Compiler-generated `copy`/`component`/default-argument bytecode on the internal view data class. JaCoCo attributes a few instructions/branches to the synthetic members with no mappable source line (no line carries `mi>0`/`mb>0`). Would be excluded if it lived under `model/`; it earns its place beside the service, so the synthetic arm is accepted. |
