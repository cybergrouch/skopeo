...
/**
 * Shared NTRP rating-calculation scenarios + level/score enumerations used by the v2 NTRP reports
 * (NtrpMatchupMatrixReport, NtrpMonteCarloReport). Curated scenarios carry literal regression deltas;
 * the level and single-set-score lists drive the exhaustive sweeps.
 */
...
@Suppress("LargeClass") // a data catalog of NTRP scenarios; size is inherent, not complexity
object TestScenarios {
            ...
            publishedCompetitionLevels.forEach { p1 ->
                publishedCompetitionLevels.forEach { p2 ->
                    sweepScores.forEach { score ->
                        add(element = generatedScenario(id = "PL$index", group = "Published", ntrpP1 = p1, ntrpP2 = p2, score = score))
                        index++
                    }
                }
            }
        ...
        buildList {
            ...
            pairs.forEach { (lower, higher) ->
                listOf(higher to lower, lower to higher).forEach { (p1, p2) ->
                    sweepScores.forEach { score ->
                        add(element = generatedScenario(id = "CG$index", group = "Competitive", ntrpP1 = p1, ntrpP2 = p2, score = score))
                        index++
                    }
                }
            }
        ...
        buildList {
            ...
            listOf("7.0" to "2.0", "2.0" to "7.0").forEach { (p1, p2) ->
                allSingleSetScores.forEach { score ->
                    add(element = generatedScenario(id = "EX$index", group = "Extreme", ntrpP1 = p1, ntrpP2 = p2, score = score))
                    index++
                }
            }
        }
    ...
    private fun expectedDeltas(
        ...
        val p1New = (p1Rating + rawChange).coerceIn(minimumValue = constants.minRating, maximumValue = constants.maxRating)
        val p2New = (p2Rating - rawChange).coerceIn(minimumValue = constants.minRating, maximumValue = constants.maxRating)
