...
class RankingPointServiceTest {
    ...
    fun `an unrated target with no explicit band is a validation error`() {
        ...
        // #921: name the player, never the internal id. Both halves matter — asserting only that the
        // name is present would still pass if the message read "player <uuid>", which is the state
        // this issue was filed about.
        error.message shouldContain "player"
        error.message shouldNotContain player.id.toString()
    }
    ...
    fun `an unrated target cannot be adjusted (#469)`() {
        ...
        // The adjust path has its own copy of the message, so it needs its own assertion (#921).
        error.message shouldContain "player"
        error.message shouldNotContain player.id.toString()
    }

    @Test
    fun `an inactive target is named, not id-ed, on both the grant and adjust paths (#921)`() {
        provision(uid = "admin", roles = setOf(Capability.PLAYER, Capability.ADMINISTRATOR))
        val player = provision(uid = "player")
        ratings.setRating(userId = player.id, rating = BigDecimal("4.3"), level = "4.0")
        users.deactivate(id = player.id).shouldBeRight()

        // Rated on purpose: it isolates the is-not-active guard from the has-no-rating one that
        // follows it, so this stays red for the right reason if the guards are ever reordered.
        val errors =
            listOf(
                service.grant(token = token(uid = "admin"), userId = player.id, request = grantRequest()),
                service.adjust(token = token(uid = "admin"), userId = player.id, request = adjustRequest()),
            ).map { it.shouldBeLeft().shouldBeInstanceOf<ServiceError.Validation>() }

        errors.forEach {
            it.message shouldContain "is not active"
            it.message shouldContain "player"
            it.message shouldNotContain player.id.toString()
        }
    }
