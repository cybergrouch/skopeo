...
data class RatingCalculationOptions(
    ...
    /**
     * Per-match-type multiplier applied to the rating change (issue #108). Different match types
     ...
     *
     * Default: 1.0 (no effect) — the persistence flow supplies the match's type factor; the
     * stateless what-if endpoint leaves it at 1.0.
     *
     * @throws IllegalArgumentException if not strictly positive
     */
    val matchTypeFactor: Double = 1.0,
) {
    init {
        require(value = smoothingFactor in 0.0..1.0) {
            "Smoothing factor must be between 0.0 and 1.0, got $smoothingFactor"
        }
        require(value = matchTypeFactor > 0.0) {
            "Match-type factor must be strictly positive, got $matchTypeFactor"
        }
    }
}
