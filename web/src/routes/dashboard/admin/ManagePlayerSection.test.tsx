...
vi.mock("@/api/generated/ratings/ratings", () => ({
  ...
  getGetApiV1UsersUserIdRatingHistoryQueryKey: (id: string) => [
    "rating-history",
    id,
  ],
}));
vi.mock("@/api/generated/capabilities/capabilities", () => ({
  ...
  usePostApiV1UsersUserIdCapabilities,
  useDeleteApiV1UsersUserIdCapabilitiesCapability,
  getGetApiV1UsersUserIdCapabilitiesQueryKey: (id: string) => [
    "capabilities",
    id,
  ],
}));
...
describe("ManagePlayerSection", () => {
  beforeEach(() => {
    ...
    usePutApiV1UsersUserIdRatings.mockImplementation(
      (options: SuccessOpts) => ({
        isPending: false,
    ...
    usePostApiV1UsersUserIdCapabilities.mockImplementation(
      (options: SuccessOpts) => ({
        isPending: false,
        mutate: (vars: unknown) => {
          grantMutate(vars);
    ...
    useDeleteApiV1UsersUserIdCapabilitiesCapability.mockImplementation(
      (options: SuccessOpts) => ({
        isPending: false,
        mutate: (vars: unknown) => {
          revokeMutate(vars);
    ...
    usePostApiV1UsersUserIdRankingPointsAdjustments.mockImplementation(
      (options: SuccessOpts) => ({
        isPending: false,
        mutateAsync: async (vars: unknown) => {
          adjustMutate(vars);
  ...
  it("shows the search picker until a player is selected", () => {
    renderSection();
    expect(
      screen.getByRole("button", { name: "pick Alice" }),
    ).toBeInTheDocument();
  ...
  it("overrides the rating and reports success", async () => {
    const user = await selectAlice();
    expect(
      (screen.getByLabelText("NTRP rating") as HTMLInputElement).value,
    ).toBe("4.000000");
  ...
  it("gates an ADMINISTRATOR grant behind a confirm step (#194)", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Confirm grant ADMINISTRATOR" }),
    );
    expect(grantMutate).toHaveBeenCalledWith({
      userId: "u1",
      data: { capability: "ADMINISTRATOR" },
    });
  });
  ...
  it("falls back to a generic message when an ADMINISTRATOR grant fails without a reason (#194)", async () => {
    ...
    await waitFor(() =>
      expect(toastError).toHaveBeenCalledWith("Could not grant the role.", {
        duration: 8000,
      }),
    );
  });
  ...
  it("surfaces the backend reason when revoking ADMINISTRATOR is refused (#194)", async () => {
      ...
      (options: SuccessOpts) => ({
        ...
        mutate: () =>
          options.mutation.onError?.({
            response: {
              data: { message: "Cannot revoke a bootstrap administrator" },
            },
          }),
      }),
    );
    ...
    await user.click(
      screen.getByRole("button", { name: "Confirm revoke ADMINISTRATOR" }),
    );
    await waitFor(() =>
      expect(toastError).toHaveBeenCalledWith(
        "Cannot revoke a bootstrap administrator",
        {
          duration: 8000,
        },
      ),
  ...
  it("rejects a missing validity date before calling the API (#469)", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Apply adjustment" }));
    expect(screen.getByRole("alert")).toHaveTextContent(
      "Both validity dates are required.",
    );
    expect(adjustMutate).not.toHaveBeenCalled();
  });
  ...
  it("surfaces the backend error when an adjustment fails (#469)", async () => {
    usePostApiV1UsersUserIdRankingPointsAdjustments.mockImplementation(
      (options: SuccessOpts) => ({
        ...
        mutateAsync: async () =>
          options.mutation.onError?.({
            response: { data: { message: "Player has no rating" } },
          }),
      }),
    );
    const user = await selectAlice();
    ...
    await user.click(screen.getByRole("button", { name: "Apply adjustment" }));
    await waitFor(() =>
      expect(toastError).toHaveBeenCalledWith("Player has no rating", {
        duration: 8000,
      }),
    );
  });
  ...
  it("surfaces the backend reason when a delete is refused, e.g. last admin (#518)", async () => {
    useDeleteApiV1UsersId.mockImplementation((options: SuccessOpts) => ({
      ...
      mutate: () =>
        options.mutation.onError?.({
          response: {
            data: { message: "Cannot delete the last active ADMINISTRATOR" },
          },
        }),
    }));
    const user = await selectAlice();
