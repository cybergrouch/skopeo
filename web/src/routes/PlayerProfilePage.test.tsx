...
vi.mock("@/components/EventsHistoryCard", () => ({
  EventsHistoryCard: ({ code }: { code?: string }) => (
    <div>events-history:{code}</div>
  ),
}));
// The standing headline has its own API hook + tests (#448); stub it here.
vi.mock("@/components/PlayerStandingCard", () => ({
  PlayerStandingCard: ({ code }: { code: string }) => (
    <div>standing:{code}</div>
  ),
}));
...
describe("PlayerProfilePage", () => {
  ...
  it("shows an error message when the player cannot be loaded", () => {
    ...
    renderAt();
    expect(
      screen.getByText(/couldn’t find or load this player/i),
    ).toBeInTheDocument();
  });
  ...
  it("shows a rating without a level", () => {
    useGetApiV1PlayersCode.mockReturnValue({
      ...
      data: {
        ...
        displayName: "Ana",
        photoUrl: null,
        rating: { value: "4.000000", level: null },
      },
    });
    renderAt();
  ...
  it("shows the active-points audit to the profile owner (#448)", () => {
    ...
    useGetApiV1UsersMe.mockReturnValue({
      data: { publicCode: "ABC234", capabilities: ["PLAYER"] },
    });
    renderAt();
  ...
  it("shows the active-points audit to an ADMINISTRATOR viewing another profile (#448)", () => {
    useGetApiV1PlayersCode.mockReturnValue(loadedPlayer);
    useGetApiV1UsersMe.mockReturnValue({
      data: { publicCode: "ZZZ999", capabilities: ["PLAYER", "ADMINISTRATOR"] },
    });
    renderAt();
    expect(screen.getByText("points-audit:ABC234")).toBeInTheDocument();
  });
  ...
  it("hides the active-points audit from a non-owner non-admin viewer (#448)", () => {
    useGetApiV1PlayersCode.mockReturnValue(loadedPlayer);
    useGetApiV1UsersMe.mockReturnValue({
      data: { publicCode: "ZZZ999", capabilities: ["PLAYER"] },
    });
    renderAt();
  ...
  it("warns the owner when their own match history is hidden (#622)", () => {
    ...
    expect(banner).toHaveTextContent(
      "Your match history is hidden from other players.",
    );
  });
  ...
  it("shows an unclaimed indicator and a claim entry point for a placeholder (#496)", () => {
    ...
    renderAt("PLH001");
    expect(
      screen.getByText("Unclaimed placeholder account"),
    ).toBeInTheDocument();
    expect(
  ...
  it("renders a merged notice linking to the canonical for a disabled duplicate (#124)", () => {
    ...
    renderAt("DUP123");
    expect(
      screen.getByText("This profile has been merged"),
    ).toBeInTheDocument();
    const link = screen.getByRole("link", {
      name: /view the active profile \(real\)/i,
    });
  ...
  it("links to a canonical without a display name (#124)", () => {
    ...
    renderAt("DUP123");
    expect(
      screen.getByRole("link", { name: /view the active profile/i }),
    ).toHaveAttribute("href", "/players/REAL99");
  });
  ...
  it("shows a merged notice without a link when the canonical is unavailable (#124)", () => {
    ...
    renderAt("DUP123");
    expect(
      screen.getByText("The active profile is unavailable."),
    ).toBeInTheDocument();
  });
