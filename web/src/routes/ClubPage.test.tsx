...
describe("ClubPage", () => {
  ...
  it("renders the name, upcoming + past event links, and a share QR", () => {
    ...
    const upcoming = screen.getByText("Upcoming events")
    ...
    expect(
      within(past).getByRole("link", { name: /Winter Cup/ }),
  ...
  it("shows each event with its type and date range (no points, #559)", () => {
    useGetApiV1ClubsCodeCode.mockReturnValue({ data: club, isLoading: false });
    renderAt();
    ...
    expect(
      within(upcoming).getByText("2026-05-01 – 2026-05-03"),
    ).toBeInTheDocument();

    const past = screen.getByText("Past events").parentElement as HTMLElement;
    expect(within(past).getByText("TOURNAMENT")).toBeInTheDocument();
    expect(
      within(past).getByText("2026-01-01 – 2026-01-03"),
    ).toBeInTheDocument();

    // No designated/awarded points copy anywhere on the page.
